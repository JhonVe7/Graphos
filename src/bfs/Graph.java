...
public class Graph {
	...
	private Integer[] validNodes = { 1, 2, 3, 4, 5, 6, 7, 8 }; // list of ALL nodes in the graph. Must be MANUALLY kept
																// in sync with the graph created in the constructor.
																// //Must be Integer arry since contains() doesn't work
																// on primiatives
	...
	// constructor initializes adjacency list with nodes from the problem
	// description
	public Graph() {
	...
	public void breadthFirstSearch(int startingNode) {
		...
		// Array saying if a node has been visited. (INDEXED FROM 1, IGNORING 0). All
		// future accesses must use an offset of +1 for "normal" arrays
		boolean[] visitedNodes = new boolean[getNodeCount() + 1];
		ArrayList<Integer> nodesInVisitedOrder = new ArrayList<Integer>(); // list of nodes in the order that they are
																			// visited. Printed at the end of the
																			// algorithm
		...
		while (!nodeQueue.isEmpty()) { // repeat the algorithm until queue is empty, like the pseudocaode says
			...
			if (!visitedNodes[nodeRemovedFromQueue + 1]) { // If NOT visited. (offset by +1 to make human readable array
															// indexes)
				...
				LinkedList<Integer> rowOfNodes = getAdjacencyList().get(nodeRemovedFromQueue); // get a list of all
																								// nodes adjacent to the
																								// current Node
				ListIterator<Integer> iterator = rowOfNodes.listIterator(); // get an iterator for the row (using
																			// iterator since LinkedList)
				while (iterator.hasNext()) { // Iterate over all nodes adjacent to the current node. The pseudocode has
												// this as a FOR LOOP, but I use an Iterator & while loop since each row
												// is a LinkedList
	...
	public void breadthFirstSearchLayers(int startingNode) {
		...
		int layerCounter = 0; // how many layers we find. Used to name the layers. Starts @ 0, but there is
								// actually 1 more layer than this number since L0 counts as the 1st.
		...
		while (!layers.get(layerCounter).isEmpty()) { // repeat until no more layers exist
			layers.add(new ArrayList<Integer>()); // create empty layer (makes an extra layer when algorithm terminate,
													// but ignored when printing)
	...
	public String toString() {
		...
		for (int i = 1; i < getNodeCount(); i++) { // iterate over the graph START @ 1, END @ <= SIZE since graph
													// representation is indexed from 1
			adjacecyListAsLines += "{" + i + "}  "; // add the index in brackets as the 1st thing on a line
			adjacecyListAsLines += getAdjacencyList().get(i) + "\n"; // add each linked list on a new line (using
																		// LinkedList built-in toString)
		}
